...
var reconnect; // = require('reconnect-core') // Manish : Currently this logic is not required
...
Connection.prototype._setupSocket = function (options) {
  ...
  var inject = reconnect(function (opts) {
    ...
    if (typeof opts.socket === 'function') {
      ...
      logger.debug('use standard socket')
...
Connection.prototype.send = function (stanza) {
  ...
  logger.debug('send: ' + stanza.toString())
...
Connection.prototype.onData = function (data) {
  logger.debug('receive: ' + data.toString('utf8'))
  if (this.parser) {
