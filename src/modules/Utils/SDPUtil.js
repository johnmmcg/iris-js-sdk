...
SDPUtil = {
    ...
    build_rtpmap: function(el) {
        var line = 'a=rtpmap:' + el.attrs.id + ' ' + el.attrs.name + '/' + el.attrs.clockrate;
        if (el.attrs.channels && el.attrs.channels != '1') {
            line += '/' + el.attrs.channels;
        }
        return line;
    },
    ...
    candidateToJingle: function(line) {
        ...
        if (line.indexOf('candidate:') === 0) {
            ...
            logger.log(logger.level.INFO, "SDPUtil", 'parseCandidate called with a line that is not a candidate line');
            logger.log(logger.level.INFO, "SDPUtil", line);
            return null;
        }
        ...
        var candidate = {},
            ...
            i;
        if (elems[6] != 'typ') {
            logger.log(logger.level.INFO, "SDPUtil", 'did not find typ in the right place');
            logger.log(logger.level.INFO, "SDPUtil", line);
            return null;
        }
        candidate.foundation = elems[0].substring(12);
    ...
    candidateFromJingle: function(cand) {
        ...
        line += cand.attrs.foundation;
        line += ' ';
        line += cand.attrs.component;
        line += ' ';

        if (RtcBrowserType.isFirefox() && cand.attrs.protocol.toLowerCase() == 'ssltcp') {
            cand.attrs.protocol = 'tcp';
        }
        line += cand.attrs.protocol; //.toUpperCase(); // chrome M23 doesn't like this
        line += ' ';
        line += cand.attrs.priority;
        line += ' ';
        line += cand.attrs.ip;
        line += ' ';
        line += cand.attrs.port;
        line += ' ';
        line += 'typ';
        line += ' ' + cand.attrs.type;
        line += ' ';
        switch (cand.attrs.type) {
            case 'srflx':
            case 'prflx':
            case 'relay':
                if (cand.attrs["rel-addr"] && cand.attrs["rel-port"]) {
                    line += 'raddr';
                    line += ' ';
                    line += cand.attrs["rel-addr"];
                    line += ' ';
                    line += 'rport';
                    line += ' ';
                    line += cand.attrs["rel-port"];
                    line += ' ';
                }
                break;
        }
        if (cand.attrs.protocol.toLowerCase() == 'tcp') {
            line += 'tcptype';
            line += ' ';
            line += cand.attrs.tcptype;
            line += ' ';
        }
        line += 'generation';
        line += ' ';
        line += cand.attrs.generation || '0';
        return line + '\r\n';
    },
        ...
        if (jingle) {
            ...
            if (contents) {
                contents.forEach(function(content, idx) {
                    desc = content.getChild('description');
                    if (desc) {
                        sources = desc.getChildren("source", "urn:xmpp:jingle:apps:rtp:ssma:0");
                    }
                });
            }
        }
