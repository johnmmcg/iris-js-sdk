...
SDPMangler = function(sdp) {
    ...
    function _formatSSRCvideo(mLine) {
        if (!mLines[mLine]) return;

        // reformatting ssrc
        var formattedSSRC = [];
        var _s = 0;
        if (mLines[mLine].ssrc[0].indexOf("cname:") > 0) {
            _s = 0;
        } else {
            _s = 1;
        }
        for (var s = _s; s < mLines[mLine].ssrc.length; s++) {
            var ssrcLine = mLines[mLine].ssrc[s];
...
SDPMangler.SerializeASBandwidth = function(sdp, bandwidth) {
    bandwidth = bandwidth || {};
...
SDPMangler.SerializePTime = function(sdp, ptimes) {
    ptimes = ptimes || {};
...
SDPMangler.SerializeGoogleMinBitrate = function(sdp, bitrate) {
    sdp = sdp.replace(/a=mid:video\r\n/g,
...
// to skip UDP ports
SDPMangler.RTPOverTCP = function(sdp) {
    return sdp.replace(/a=candidate:.*udp.*\r\n/g, '');
};
module.exports = SDPMangler;
